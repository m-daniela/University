...
class RDParser:
    def __init__(self, grammar, configuration, filename):
    ...
    def read_input(self, filename):
        with open(filename, "r") as f:
            lines = f.readlines()
        for line in lines:
            self.input_seq.append(line.strip().split(" ")[0])

        # with open("files/yes.txt", "w") as g:
        #     g.write(str(self.input_seq))
        ...
        while self.configuration.state != "f" and self.configuration.state != "e":
            ...
            if self.configuration.state == "q":
                ...
                if len(self.configuration.input_stack) == 0 and self.configuration.i == n:
                    ...
                    if not (self.configuration.i < n and len(self.configuration.input_stack) == 0):
                        top = self.configuration.input_stack[0]
                        if top in self.grammar.non_term:
                            self.expand()
                        else:
                            # if top input = current symbol in w and current position is lower than the length of input
                ...
                if self.configuration.state == "b":
                    if not (self.configuration.i == 0 and len(self.configuration.working_stack) == 0):
    ...
    def error(self):
        self.configuration.state = "e"
        print(f"Error {self.configuration}")