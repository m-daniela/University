...
from flcd.Lab4.lab4 import FiniteAutomata


class Scanner:
    def __init__(self, symbol_table):
        self.fa_nr = FiniteAutomata()
        self.fa_ident = FiniteAutomata()
        self.fa_str = FiniteAutomata()
        self.fa_char = FiniteAutomata()
        ...
        self.read_finite_automata()

    def read_finite_automata(self):
        self.fa_ident.read_file("lab4/identifier.in")
        self.fa_nr.read_file("lab4/constant_nr.in")
        self.fa_str.read_file("lab4/constant_string.in")
        self.fa_char.read_file("lab4/constant_char.in")
    ...
    def import_reserved(self, filename="files/token.in"):
        ...
        for i in range(5, len(lines)):
            element, code = lines[i].strip().split(" ")
            if i < 20:
                operators.append([element, code])
            elif i < 28:
                self.__separators[element] = code
            elif i > 28:
                self.__reserved_words[element] = code

        for operator in operators:
            if len(operator[0]) > 1:
                self.__compound_operators[operator[0]] = operator[1]
            else:
                self.__simple_operators[operator[0]] = operator[1]
    ...
    def check_identifier(self, token):
        return self.fa_ident.is_accepted(token)
    ...
    def check_constant(self, token):
        return self.fa_char.is_accepted(token) or self.fa_str.is_accepted(token) or self.fa_nr.is_accepted(token)
        ...
        for line_nr, line in enumerate(self.__lines):
            try:
                ...
                for token in parsed_line:
                    if token != "":
                        if token in self.__reserved_words.keys():
                            self.__pif.append((self.__reserved_words[token], -1))
                        elif token in self.__separators.keys():
                            self.__pif.append((self.__separators[token], -1))
                        elif token in self.__compound_operators.keys():
                            self.__pif.append((self.__compound_operators[token], -1))
                        elif token in self.__simple_operators.keys():
                            self.__pif.append((self.__simple_operators[token], -1))
                        else:
                            if self.check_identifier(token):
                                self.__symbol_table.insert(token)
                                self.__pif.append((0, self.__symbol_table.get_position(token)))
                            elif self.check_constant(token):
                                self.__symbol_table.insert(token)
                                self.__pif.append((1, self.__symbol_table.get_position(token)))
                            else:
